    ...
    <div class="flex flex-wrap gap-2 sm:gap-3 mt-4 sm:mt-0">
        <?php if ($showInactive): ?>
            <a href="pacientes.php" class="btn btn-secondary">
                ...
                <span class="hidden sm:inline">Ocultar Inativos</span>
            </a>
        <?php else: ?>
            <a href="pacientes.php?show_inactive=1" class="btn btn-secondary">
                <i class="fas fa-eye"></i>
                <span class="hidden sm:inline">Mostrar Inativos</span>
            </a>
        <?php endif; ?>
...
<div id="pacientes-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
        ...
        <div class="col-span-full">
            <div class="card-shadow">
                <div class="empty-state py-12">
                    ...
                    <a href="cadastro.php" class="btn btn-primary mt-4">
    ...
    <?php else: ?>
        ...
        <div class="card-shadow overflow-hidden group" 
             data-searchable="<?php echo htmlspecialchars(strtolower($paciente['nome'] . ' ' . $paciente['whatsapp'] . ' ' . $paciente['email'])); ?>">
            <!-- Foto/Header -->
            <div class="relative h-40 sm:h-48 bg-gradient-to-br from-primary/5 to-accent/5 overflow-hidden">
                <?php if (!empty($paciente['foto']) && file_exists(__DIR__ . '/uploads/pacientes/' . $paciente['foto'])): ?>
                    <img src="uploads/pacientes/<?php echo $paciente['foto']; ?>" 
                ...
                <div class="absolute top-3 right-3">
                    <?php if ($paciente['ativo']): ?>
                        <span class="badge badge-success">
                            <i class="fas fa-check text-xs"></i> Ativo
                        </span>
                    <?php else: ?>
                        <span class="badge badge-danger">
                            <i class="fas fa-times text-xs"></i> Inativo
                        </span>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="p-4 sm:p-5">
                ...
                <div class="flex gap-2">
                    <a href="paciente_editar.php?id=<?php echo $paciente['id']; ?>" 
                       class="flex-1 bg-gray-100 hover:bg-primary hover:text-white text-text-main text-center py-2.5 rounded-lg font-semibold transition-all flex items-center justify-center gap-2">
                        <i class="fas fa-edit"></i>
                        <span>Editar</span>
                    </a>
                    <a href="paciente_excluir.php?id=<?php echo $paciente['id']; ?>" 
                       onclick="return confirm('Tem certeza que deseja excluir este paciente? Esta ação não pode ser desfeita.');" 
                       class="w-11 h-11 bg-red-50 hover:bg-danger hover:text-white text-danger rounded-lg font-semibold transition-all flex items-center justify-center">
...
<!-- Empty Search Message (hidden by default) -->
